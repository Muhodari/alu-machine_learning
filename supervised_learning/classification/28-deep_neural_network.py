#!/usr/bin/env python3
"""
DeepNeuralNetwork class for multiclass classification with different activation functions
"""
...
class DeepNeuralNetwork:
    ...
    def activation(self):
        ...
        Returns:
            str: The activation function type
        """
    ...
    def _sigmoid(self, Z):
        ...
        Returns:
            numpy.ndarray: Sigmoid activation
        """
        return 1 / (1 + np.exp(-Z))
    ...
    def _sigmoid_derivative(self, A):
        ...
        Args:
            A (numpy.ndarray): Activated values

        Returns:
            numpy.ndarray: Sigmoid derivative
        """
        return A * (1 - A)
    ...
    def forward_prop(self, X):
        ...
        Returns:
            tuple: (output, cache)
                output: numpy.ndarray with shape (classes, m) containing the output of the network
    ...
    def cost(self, Y, A):
        ...
        Args:
            Y (numpy.ndarray): Correct labels with shape (classes, m)
        ...
        # Calculate the cost using logistic regression formula for multiclass
    ...
    def evaluate(self, X, Y):
        ...
        Returns:
            tuple: (prediction, cost)
                prediction: numpy.ndarray with shape (classes, m) containing predicted labels
                cost: float representing the cost of the network
        """
        # Perform forward propagation to get activated outputs
        A, _ = self.forward_prop(X)

        # Convert probabilities to one-hot predictions
    ...
    def gradient_descent(self, Y, cache, alpha=0.05):
        ...
        for l in range(self.__L - 1, 0, -1):
            ...
            if self.__activation == 'sig':
                dZ = np.dot(self.__weights[f'W{l+1}'].T, dZ) * self._sigmoid_derivative(cache[f'A{l}'])
            else:  # tanh
                dZ = np.dot(self.__weights[f'W{l+1}'].T, dZ) * self._tanh_derivative(cache[f'A{l}'])
